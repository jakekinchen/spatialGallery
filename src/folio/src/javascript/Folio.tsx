...
const Folio = () => {
        ...
        window.addEventListener(
            ...
            () => {
                ...
                passes.current.horizontalBlurPass.material.uniforms.uStrength.value =
                    new THREE.Vector2(
                        passes.current.horizontalBlurPass.strength,
                        0,
                    );
                ...
                passes.current.verticalBlurPass.material.uniforms.uStrength.value =
                    new THREE.Vector2(
                        0,
                        passes.current.verticalBlurPass.strength,
                    );
            },
            {
                once: true,
        ...
        time.current.on("tick", () => {
            if (world.current && world.current.car) {
                camera.current.target.x =
                    world.current.car.chassis.object.position.x;
                camera.current.target.y =
                    world.current.car.chassis.object.position.y;
            }
        });
    }, []);
    const setPasses = useCallback(() => {
        ...
        passes.current.renderPass = new RenderPass(
            scene,
            camera.current.instance,
        );
        passes.current.horizontalBlurPass = new ShaderPass(BlurPass);
        passes.current.horizontalBlurPass.strength = config.current.touch
            ? 0
            : 1;
        ...
        if (debug.current) {
            ...
            folder
                .add(
                    passes.current.horizontalBlurPass.material.uniforms
                        .uStrength.value,
                    "x",
                )
                .step(0.001)
            ...
            folder
                .add(
                    passes.current.verticalBlurPass.material.uniforms.uStrength
                        .value,
                    "y",
                )
                .step(0.001)
        ...
        if (debug.current) {
            ...
            folder
                .add(
                    passes.current.glowsPass.material.uniforms.uRadius,
                    "value",
                )
                .step(0.001)
                .min(0)
        ...
        time.current.on("tick", () => {
            passes.current.horizontalBlurPass.enabled =
                passes.current.horizontalBlurPass.material.uniforms.uStrength
                    .value.x > 0;
